...
import com.enji_iot.mqtt.Service.MailService_MQTT;
...
//			dao.insert("IotAlarmInfo.insert", alarm);
			...
			mqttMapper.insertIotAlarmInfo(alarm);
		} catch (Exception e) {
			LogUtil.errorLog(e);
		}
	}
...
//			dao.insert("IotHistoryTriggerInfo.insert", history);
			MQTTMapper mqttMapper = (MQTTMapper)SpringApplicationContext.getBean("MQTTMapper");
			mqttMapper.insertIotHistoryTriggerInfo(history);
		} catch (Exception e) {
			LogUtil.errorLog(e);
		}
	}
...
//					dao.update("UserAccountInfo.updatevoicenum", userAccountBo) ;
					MQTTMapper mqttMapper = (MQTTMapper)SpringApplicationContext.getBean("MQTTMapper");
					mqttMapper.updatevoicenum(userAccountBo);
...
//					dao.insert("AlarmTriggerRecord.insert", alarmTriggerRecord) ;
					mqttMapper.insertAlarmTriggerRecord(alarmTriggerRecord);
				}
			});
		}else{
...
//			contactUserInfo = dao.selectOne("ContactUserInfo.selectOne", contactUserInfo);
			MQTTMapper mqttMapper = (MQTTMapper)SpringApplicationContext.getBean("MQTTMapper");
			contactUserInfo = mqttMapper.selectOne(contactUserInfo);
...
//					dao.update("UserAccountInfo.updatesmsnum", userAccountBo) ;
					MQTTMapper mqttMapper = (MQTTMapper)SpringApplicationContext.getBean("MQTTMapper");
					mqttMapper.updatesmsnum(userAccountBo);
					// 增加用户触发
					AlarmTriggerRecordBO alarmTriggerRecord = new AlarmTriggerRecordBO() ;
					alarmTriggerRecord.setUser_id(add_user_id);
...
//			contactUserInfo = dao.selectOne("ContactUserInfo.selectOne", contactUserInfo);
			...
			if (ObjectUtil.isNotEmpty(contactUserInfo) && ObjectUtil.isNotEmpty(contactUserInfo.getEmail())) {
				...
				MailService_MQTT mailService = (MailService_MQTT) SpringApplicationContext.getBean("mailServiceImpl");
				if(flag == 1){
